...
function describeEstate(estate) {
  ...
  let gardens =
    describeGarden("Rose Arbor", estate.roseArbor) +
    describeGarden("Perennial Garden", estate.perennialGarden) +
 ...
 */
//Tell the computer how to access the gallonsWaterPerWeek
// Total the number of gallons for all plants on the estate
// reasign numGalllons varable to total amount we've found

function calculateWaterUsagePerWeek(estate) {
  let numGallons = 0;
  for (let i = 0; i < Object.keys(estate).length; i++) {
    let garden = Object.values(estate)[i];
    // let garden = [plant, plant, plant]

    for (let plant = 0; plant < garden.length; plant++) {
      let amountWater = Object.values(garden)[plant].gallonsWaterPerWeek * 10;
      // let amountWater = plant.gallonsWaterPerWeek;

      numGallons += amountWater;
    }
  }
...
//ESTATE -> roseArbor, perennialGardens, slopePlanters
//GARDEN -> [plant, plant, plant]
//PLANT -> type, flowerColor, gallonsWaterPerWeek

//ESTATE(Obj) -> 3 GARDENS(Arr) -> PLANTS(Obj) -> Property

//gallonsWaterPerWeek = number
//plant.gallonsWaterPerWeek
